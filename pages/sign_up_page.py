...
class SignUpPage(BasePage):
    ...
    def email_id_input_2(self) -> WebElement:
        return self.find_element(SignUpLocators.SECOND_EMAIL_INPUT)
    ...
    def cant_send_email_text(self) -> WebElement:
        return self.find_element(SignUpLocators.SEND_EMAIL_ERROR_TEXT)
    ...
    def check_account_create(self):
        self.find_element(SignUpLocators.SEND_EMAIL_ERROR_TEXT)
    ...
    def check_new_account_log_in(self):
        self.find_element(SignUpLocators.CONFIRM_ACCOUNT_TEXT)
        element = self.find_elements(SignUpLocators.SEND_EMAIL_ANEW_BUTTON)
        if len(element) > 0:
            return True
        return False
